"""
    Preamble:
        ...
        This class represents vector based implementation of Binary Min Heap.

        Parameters:
            1) heapList : List[] --> List to store the elements of heap.
            2) sz : int --> Represents the size of the heap.

        Methods:
            1) (method) def upHeap(self: Self@BinaryMinHeap, i: Any) -> None :- Upheap the element at a given index in the heapList to restore heap order property.

            2) (method) def insert(self: Self@BinaryMinHeap, val: Any) -> None :- Inserts a new element into the heap and restores the heap order propery.

            3) (method) def minChild(self: Self@BinaryMinHeap, i: Any) -> int :- Returns index of minimum child of a given element.

            4) (method) def downHeap(self: Self@BinaryMinHeap,i: Any) -> None :- Downheaps the element at a given element to restore heap order property.

            5) (method) def delMin(self: Self@BinaryMinHeap) -> int :- Deletes the minimum element, restores heap order property and returns the minimum element.
"""
class BinaryMinHeap:
    ...
    def downHeap(self,i): # O(log(n))
        ...
        while i * 2 <= self.sz:
            mc = self.minChild(i) # Get minChild to swap it with the current element if we have to
            if self.heapList[i] > self.heapList[mc]: # Swap current element with minChild if it is greater than minChild
    ...
    def delMin(self): # O(log(n))
        ...
        """
        if self.sz == 0 or len(self.heapList) == 1:  # Check if heap is empty
            return "Heap is empty!!"
        
        minElement = self.heapList[1]
        if self.sz == 1:  # If only one element is left, simply remove it
            self.heapList.pop()
        else:
            last = self.heapList.pop()
            self.heapList[1] = last  # Replace root with rightmost leaf in last level.
        self.sz -= 1 
        if self.sz > 0:  # Only downHeap if the heap is not empty after deletion
            self.downHeap(1) 
        return minElement
