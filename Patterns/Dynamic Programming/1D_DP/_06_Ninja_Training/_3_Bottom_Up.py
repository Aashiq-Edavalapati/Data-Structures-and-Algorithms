...
def ninjaTraining(points: List[List[int]]) -> int:
    ...
    for prev in range(4):
        for task in range(3):
            if task != prev:
                dp[0][prev] = max(dp[0][prev], points[0][task])
    ...
    return dp[len(points) - 1][3] # Because, the initial call in the top-down approach will be the final answer
...
if __name__ == '__main__':
    ...
    for i, points in enumerate(testCases):
        print(f'TestCase {i}: i/p: {points} o/p: {ninjaTraining(points)}')