...
"""Read the comments with respect to the code from _1_Recursion.py(Brute Force) and Top Down"""
def unboundedKnapsack(wt, val, n, W):
    dp = [[0 for _ in range(W + 1)] for _ in range(n)] # Initialize DP table of same size as in memoization
    ...
    for idx in range(n): # idx (0 to n - 1)
        for w in range(W + 1): # w (0 to W)
            ...
            if wt[idx] <= w: # Pick only if weight of current element <= Weight of the bag
                pick = val[idx] + dp[idx][w - wt[idx]] # Increase profit by val[idx] and stay at the same because each element can be taken any number of times. Update remaining capacity of the bag!

            # Choice 2: Don't pick the current element
            notPick = 0
            if idx > 0: # Index bound check
                notPick = dp[idx - 1][w] # Move to the next element

            dp[idx][w] = max(pick, notPick) # Choose maximum of both the paths
