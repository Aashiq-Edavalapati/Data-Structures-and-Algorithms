...
def change(amount: int, coins: List[int]) -> int:
    ...
    for target in range(amount + 1):
        prevRow[target] = 1 if target % coins[0] == 0 else 0 # idx == 0
    ...
    for idx in range(1, n): # idx == 0 is a base case => Start from 1
        ...
        for target in range(1, amount + 1): # target == 0 is a base case => Start from 1
            ...
            if target - coins[idx] >= 0:
                pick = currRow[target - coins[idx]] # dp[idx] => currRow
            notPick = prevRow[target] # dp[idx - 1] => prevRow

            currRow[target] = pick + notPick # dp[idx] => currRow
        prevRow = currRow # Change the prevRow to currRow
