...
def rodCutting(price: List[int], n: int) -> int:
    dp = [[0 for _ in range(n + 1)] for _ in range(n + 1)] # Initialize DP table
    for curr in range(n, 0, -1): # Start from n to 1 (Opposite direction of recursion)
        for rem in range(curr, n + 1): # Start from curr because, anyway if rem < curr => return 0
            pick = price[curr - 1] + dp[curr][rem - curr] # f(curr, rem - curr) => dp[curr][rem - curr]
            notPick = 0
            if curr < n: # Index bound check
                notPick = dp[curr + 1][rem] # f(curr + 1, rem) => dp[curr + 1][rem]

            # Store the result in DP table
            dp[curr][rem] = max(pick, notPick)
        
    return dp[1][n] # f(1, n) => dp[1][n]
