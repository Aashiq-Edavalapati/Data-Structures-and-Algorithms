...
def maxCandies(grid: List[List[int]]) -> int:
    ...
    for j1 in range(m):
        for j2 in range(m):
            dp[n - 1][j1][j2] = grid[n - 1][j1] + grid[n - 1][j2] if j1 != j2 else grid[n - 1][j1]
                ...
                for d1 in range(-1, 2):
                    for d2 in range(-1, 2):
                        if 0 <= j1 + d1 < m and 0 <= j2 + d2 < m:
                            if j1 == j2:
                                rec = grid[i][j1] + dp[i + 1][j1 + d1][j2 + d2]
                            else:
                                rec = grid[i][j1] + grid[i][j2] + dp[i + 1][j1 + d1][j2 + d2]
                        else:
                            rec = minVal
                        dp[i][j1][j2] = max(dp[i][j1][j2], rec)
